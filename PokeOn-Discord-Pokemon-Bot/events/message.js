...
module.exports = async (client, message) => {
    ...
    let prefixs = prefix;
    let botacc = await User.findOne({id: '836076205719289876'})
    ...
    if ([`<@${client.user.id}>`, `<@!${client.user.id}>`].includes(message.content) && !args[0]) {
        ...
        return message.channel.send(`Hi , My prefix is ${guild.prefix}`)
    }

    if (!args[0]) return;
    const command = args.shift().toLowerCase();
    const cmd = client.commands.get(command) || client.commands.find(cmd => cmd.aliases && cmd.aliases.includes(command));
    if (message.content.toLowerCase().startsWith(prefix) && guild.blacklist == true) return message.channel.send("This server has been blacklisted. Join support server to appeal.");
    if (user && user.blacklist == true && message.content.toLowerCase().startsWith(prefix)) return message.channel.send("You have been blacklisted. Join support server to appeal.");
...
async function spawnPokemon(message, client) {
    ...
    await get(options).then(async t => {
        ...
        if (poke.url.endsWith(".gif")) imgname = "Pokemon.gif"
...
async function leveling(message, client) {
    // if (!guild.levelupbtn) return; this is what which is creating problems >_<
    let user = await User.findOne({ id: message.author.id });
    if (!user) return;
    if (user.blacklist) return;
    let selected = user.selected;
    let poke = user.pokemons[selected]; // not a good method to sort up :(
    if (!poke) return
    ...
    if (guild.disabledChannels.includes(message.channel.id)) return;
    //if (!channel) return; // bro wtf you did ?
    ...
    // (1.2 * lvl ^ 3) - (15 * lvl ^ 2) + (100 * lvl) - 140 and wtf is this?
    if (user.blacklist) return;
    if (newXp > neededXp) {
        poke.level = lvl + 1;
        poke.xp = 0;
