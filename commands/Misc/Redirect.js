...
module.exports = {
    ...
    if(!nchannel) return message.channel.send(`> ${config.no} Correct usage: **${nguild.prefix || client.config.prefix}redirect <mentionchannel/id/name>**`);
      ...
      await nguild.save();
      return message.channel.send(`> ${config.yes} **PokéOn will now redirect all spawns to <#${nguild.spawnchannel}> To allow spawns in all channels again, type **\`${prefix}redirect disable.\``);
  }
}   
