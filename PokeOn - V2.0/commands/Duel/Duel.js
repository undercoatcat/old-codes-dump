...
//HP = floor(0.01 x (2 x Base + IV + floor(0.25 x EV)) x Level) + Level + 10
                ...
                //let hp = Math.floor(Math.floor(0.01*(2*user.pokemons[selected].hp))))
                ...
                async function RunDuel() {
                    ...
                    used2 = null;
//damage eq. 
                  let A = 2*user.pokemons[selected].level+10
                  let ADI = A/250
                  let Pokename = user.pokemons[selected].name;
                  let Poke2name = user2.pokemons[selected1].name;

                   let response = await P.getPokemonSpeciesByName(Pokename)
    const options = {
      url: `https://pokeapi.co/api/v2/pokemon/${Pokename}`,
      json: true
    }

                  let ATK = Pokename.atk;
                  let Def = Poke2name.def

  

                  AD = ATK/DEF;

                  let ADIXATDE = ADI*AD + 2;

                  let Rand = Math.floor(Math.random() * 1) + 0.85;
